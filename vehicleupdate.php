...
if (isset($_POST['update'])) 
  { 
    ...
    $id=$_POST['id'];
    $query="SELECT * FROM jeep WHERE vehicle_id='$id'";
...
?>
              ...
              <h4><a href="">Update Vehicle</a></h4>
          <div id="login">
          <form id="login"  method="post"  enctype="multipart/form-data">
          <div class="form-outline mb-4">
            <input type="text" id="plate" value="<?php echo $fetch['plate_num']; ?>" class="form-control" name="plate_number" required><label class="form-label text-light" for="plate">Plate No.</label>            
          </div>
          <div class="form-outline mb-4">
            <input type="text" id="vname" value="<?php echo $fetch['vehicle_name']; ?>" class="form-control" name="vname" required><label class="form-label text-light" for="vname">Vehicle Name</label>            
          </div>
          <div class="form-outline mb-4">
            <input type="text" id="vtype" value="<?php echo $fetch['v_type']; ?>" class="form-control" name="vtype" required><label class="form-label text-light" for="vtype">Vehicle Type</label>            
          </div>
          <div class="form-outline mb-4">
            <input type="text" id="capacity" value="<?php echo $fetch['capacity']; ?>" class="form-control" name="capacity" required><label class="form-label text-light" for="capacity">Capacity</label>            
          </div>          

          <input type="hidden" name="vehicle_id" value="<?php echo $fetch['vehicle_id']; ?>">  
         <input type="submit" name="update_btn" value="Update Vehicle!"> 

<?php
}
if(isset($_POST['update_btn'])) {
  $vehicle_id = trim($_POST['vehicle_id']);
  $plate_num = trim($_POST['plate_number']);
  $vname = trim($_POST['vname']);
  $vtype = trim($_POST['vtype']);
  $capacity = trim($_POST['capacity']);

  $sql = "CALL up_jeep('$vehicle_id','$plate_num','$vname','$vtype','$capacity')";

if(pg_query($con,$sql)) {
    
    echo "<script> alert('Successfully Updated'); window.location='adminvehicle.php'; </script>" ;

} else {
    echo "<script> alert('Failed to Update/Edit Vehicle (Missing components or fill ups)'); </script>";
