...
?>
          ...
          <div class="form-outline mb-4">
            <input type="text" id="typeemail" class="form-control" name="dlicense" required><label class="form-label text-light" for="typeemail">License No.</label>            
          </div>
...
}  elseif(empty($dlicense)) {
    echo "<p> Please enter Driver's License Number </p>";
} else {
  $sql = "CALL insert_driver('$fname','$lname','$address','$dlicense');"; 
  $query = pg_query($con, $sql);
                    $result = pg_fetch_array($query1);
                    echo "<script> alert('Driver Successfuly Registered'); window.location='admindriver.php'; </script>";                
  }
}
?>
